...
export class EditCompanyComponent implements OnInit {
      ...
      for (var index in res.data) 
      {
        if(this.user_id == res.data[index]._id)
        {
          ...
          this.editform = new FormGroup({
            ...
            email: new FormControl(res.data[index].default_person_met.email),
            mobile: new FormControl(res.data[index].default_person_met.mobile),
  ...
  onUpdate()
  {
    ...
    .pipe(first())
    .subscribe((data) => {
        if(data.message=="Company details successfully updated") 
        {
          this.showupdate();
